...
1. **arr[0..left-1] < target**
   - 左边所有位置都 **小于 target**
   - 因为每次 arr[mid] < target，我们才把 left = mid+1，确保左边都是小于 target 的元素（left-1永远小于target）
2. **arr[right+1..n-1] >= target**
   - 右边所有位置都 **大于或等于 target**
   - 因为每次 arr[mid] >= target，我们才把 right = mid-1，确保右边都是 ≥ target 的元素（right+1永远大于target）

**循环结束时**，条件是 left > right，left= right+1：
